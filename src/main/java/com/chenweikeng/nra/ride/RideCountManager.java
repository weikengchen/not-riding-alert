...
public class RideCountManager {
    ...
    private static final File DATA_FILE = new File(FabricLoader.getInstance().getConfigDir().toFile(), "not-riding-alert-rides.json");
    ...
    /**
     * Updates the ride count for a given ride.
     * Returns true if the count increased (indicating a change that should trigger a save).
     */
    ...
    /**
     * Checks if there are any changes (increases) and saves if needed.
     * Should be called periodically (every tick or every few seconds).
     */
